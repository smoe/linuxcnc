...
It offers the possibility to display up to 9 axes, support a lathe mode for normal and back tool lathe and can be adapted to nearly every need,
...
so there is no need to translate unneeded stuff. The files are placed in */src/po/gmoccapy*.
...
+
[NOTE]
If no path or file is given, GMOCCAPY will use as default <your_machinename>.pref,
if no machine name is given in your INI File it will use gmoccapy.pref.
...
* EMBED_TAB_COMMAND = The command to execute, e.g.,
+
----
...
Connections to GMOCCAPY HAL pins need to be made in the postgui HAL file specified in your INI file, because these pins do not exist prior of realizing the GUI.
...
The float pins do accept values from 0.0 to 1.0, being the fraction at which you want to set the slider value.

[WARNING]
If you use both connection types, do not connect the same slider to both pins,
as the influences between the two has not been tested!
...
For the standard XYZ config the following HAL pins will be available:
...
[NOTE]
====
Messages or user infos will not affect the `gmoccapy.error` pin,
but the `gmoccapy.delete-message` pin will delete the last message if no error is shown!
====
...
# not needed on every machine configuration
...
* _Start as window_  - If you select start as window the spinboxes to set the position and size will get active.
  ...
  Nevertheless the user can change the size and position using the mouse,
  but that will not have any influence on the settings.
...
Or an other user having a MPG Wheel with 500 ipr and he wants to set the spindle override witch has limits from 50% to 120%,
...
If you give a maximum of 2000 and your spindle makes 4000 rpm,
only the bar level will be wrong on higher speeds than 2000 rpm.
...
* _Use auto tool measurement_ - If checked, after each tool change, a tool measurement will be performed,
...
This will set the corresponding button insensitive (grayed out),
...
* _Use frames_ - If you activate the checkbox, each message will be displayed in a frame, so it is much easier to distinguish the messages.
  But you will need a little bit more space.
...
It has made a wrong entry in the INI file under the `[TRAJ]`, i.e., `MAX_LINEAR_VELOCITY =` _xxx_. Change that entry to `MAX_VELOCITY =` _xxx_.
