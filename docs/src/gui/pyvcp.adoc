...
A LED is used to indicate the status of a 'bit' HAL pin.
The LED color will be on_color when the HAL pin is true, and off_color otherwise.
...
A text button controls a 'bit' HAL pin.
The HAL pin is false until the button is pressed then it is true.
...
A checkbutton controls a bit HAL pin.
The HAL pin will be set True when the button is checked, and false when the button is unchecked.
...
A radiobutton will set one of the HAL pins true. The other pins are set false.
...
dependent upon the value of the HAL pin (range1, range2 range3 must all be set, if you only want 2 ranges, set 2 of them to the same color).
...
The 'image_bit' toggles between two images by setting the HAL pin to true or false.
...
you 'select' the image by setting the HAL pin to a integer value with 0 for the first image in the images list and 1 for the second image, etc.
...
.Simple image_u32 Example with halpin=1
image::images/pyvcp_image01.png[]

.Simple image_u32 Example with halpin=2
image::images/pyvcp_image02.png[]
