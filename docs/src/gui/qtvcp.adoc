...
QtVCP is completely _customizable_: you can add different buttons and status LEDs, etc. or add Python code for even finer grain customization.
...
To see available themes, they can be loaded by running the following
...
_PATH_ can be any valid path. It can use `~` for home directory or `WORKINGDIRECTORY` or `CONFIGDIRECTORY` to represent QtVCP's idea of those directories, e.g.: +
...
The default path is in the configuration directory as a hidden file using the screen basename and rc, i.e.: *`CONFIGDIRECTORY/.<screen_name>rc`*
...
QtVCP usa QT5 con python2. Esta combinación normalmente no está disponible desde repositorios.
Puede compilarlo usted mismo, o hay versiones precompiladas disponible para sistemas comunes.
En 'lib/python/qtvcp/designer' hay carpetas basadas en arquitecturas de sistema y luego la versión QT.
...
Pick _'Main Window'_ and press the _'Create'_ button. + A _`MainWindow` widget_ is displayed.
