...
rich https://www.gtk.org/[GTK+] widget set, and in fact all of these
...
- Widget set: relies on the  https://www.gtk.org/[GTK+] widget set.
...
information and a tutorial on Glade, see https://glade.gnome.org. Some
...
https://www.pygtk.org/docs/pygtk/class-gtkbutton.html[GtkButton] in the
https://www.pygtk.org/docs/pygtk[PyGtk Reference Manual].
...
It assumes a running LinuxCNC environment like AXIS or Touchy. To connect to it, inspects the INI_FILE_NAME environment variable.
...
If you run it in a GladeVCP application when LinuxCNC is not running,
you might get a traceback because the Gremlin widget can't find LinuxCNC status, like the current file name.
...
- Describe these decisions in your handler class' `\__init()__` method
...
_without_ saving state. To make sure that this case is covered, add
...
Since GladeVCP uses Gtk widgets,
which rely on the https://www.pygtk.org/pygtk2reference/gobject-functions.html[GObject] base class, the full glib functionally is available.
...
In the following example (assuming a HAL Meter widget called `meter`),
the meter's min value is set from an INI file parameter at startup,
...
#FIXME: URL https://osdir.com/ml/gtk-app-devel-list/2010-04/msg00129.html is no longer available
I defined a `hal_spinbutton` widget in Glade, and set a default `value` property in the corresponding adjustment. It comes up with zero?::
  ...
  https://osdir.com/ml/gtk-app-devel-list/2010-04/msg00129.html does
...
- Run GladeVCP or AXIS from a terminal window. If you get Python errors,
  ...
  `/usr/lib/python2.6/dist-packages/_hal.so` (note the underscore); if yes,
