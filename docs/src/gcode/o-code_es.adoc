...
== Comentarios[[ocode:comments]](((Comments)))
...
== Subrutinas[[ocode:subroutines]](((Subrutinas)))
...
valor como en el contexto de llamada.
Al regresar de la subrutina, los valores de
...
=== Programas numerados al estilo Fanuc[[ocode:fanuc-style-programs]](((Subrutinas, M98, M99)))
...
y no puede terminar una subrutina que comience con
sintaxis `o100 sub`.
...
* Los parámetros `#1`,` #2`, ..., `#30` son globales y accesibles en
  subprogramas numerados, similares a los parámetros numerados más altos
  en llamadas de estilo tradicional. Modificaciones a estos parámetros
  dentro de un el subprograma son modificaciones globales y persistirán
  después del retorno de subprograma.

* Las llamadas `M98` a subprograma no tienen valor de retorno.

* Los bloques de llamadas del subprograma `M98` pueden contener una palabra L opcional especificando un recuento de repetición de bucle.
  Sin la palabra L, el subprograma se ejecutará solo una vez (equivalente a `M98 L1`).
  Un bloque `M98 L0` no ejecutará el subprograma.
...
Tenga en cuenta que el parámetro `#1` es global.
Al final del programa principal, después de las actualizaciones dentro de `O100` y `O200`, su valor será igual a `5.25`.

== Bucles[[ocode:looping]](((Subrutinas, bucles)))
...
== Condicionales[[ocode:conditional]](((Subrutinas, bucles condicionales)))
...
== Repeat[[ocode:repeat]](((Subrutinas, repetir bucle)))
...
== Indirección[[ocode:indirection]](((Indirección)))
...
== Llamando a archivos[[ocode:calling-files]](((Archivos de llamada)))
...
== Errores[[ocode:errors]](((Errores de código O)))
...
Para hacer estos errores advertencias no fatales en stderr,
establezca el bit 0x20 en la opción mask '[[RS274NGC]FEATURE=' del .ini.
